...
int main(void) {
  ...
  for (auto filename : Fs) {
    ...
    string show_name =
        ((filename.rfind(prefix, 0) == 0) ? filename.erase(0, prefix.length())
                                          : filename);
    ...
    // Mostra resultado
    cout << left << setw(25) << "Image:" << show_name << '\n'
         << left << setw(25) << "Native percentage:" << native_percentage
         << '\n'
         << left << setw(25)
         << "Agriculture percentage:" << agriculture_percentage << "\n"
         << "-----------------------------\n";
  }
